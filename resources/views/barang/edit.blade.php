        ...
        <div class="card-body">
            <form method="POST" action="{{ url('/barang/' . $barang->barang_id) }}" class="form-horizontal">
                ...
                <div class="form-group row">
                    ...
                    <div class="col-11">
                        <input type="number" class="form-control" id="harga_jual" name="harga_jual" value="{{ old('harga_jual', $barang->harga_beli) }}" required>
                        @error('harga_beli')
                            <small class="form-text text-danger">{{ $message }}</small>
                        @enderror
                    </div>
                    ...
                    <div class="col-11">
                        <select class="form-control" id="kategori_id" name="kategori_id" required>
                            ...
                            @foreach ($kategori as $item)
                                <option value="{{ $item->id }}" @if ($item->id == $barang->kategori_id) selected @endif>{{ $item->nama_kategori }}</option>
                            @endforeach
                        </select>
                        @error('kategori_id')
                            <small class="form-text text-danger">{{ $message }}</small>
